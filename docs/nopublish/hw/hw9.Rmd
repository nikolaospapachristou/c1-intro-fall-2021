...
* Combine these three data sets into one data set as a new object named `lotr`
...
* Have a meaningful name (do not use spaces or capital letters)
* Tidy the combined data set
* Save the tidy data to the data folder in your project as `tidy_lotr`

```{r tidy_data}

tidy_lotr <- lotr_untidy %>% 
  pivot_longer(
    cols = c(Female, Male),
    names_to = "Gender",
    values_to = "Words"
...
export(lotr_tidy, here("data", "tidy_lotr.csv"))
```
<br>
<br>
...
* Tidy these data to reproduce exactly the `tidy_lotr` data from above
...
* Tidy the remaining dataset such that a case represents a score and percentage within a unit (i.e., *score* and *percentage* will be separate columns). 
  + Take a VERY close look at the data. 
  + You will be able to use the `names_sep` argument if you **first** `rename()` two columns. Identify those two columns first. 
  + Also, you will need to `pivot_longer` and then `pivot_wider` in order to tidy properly. 
  + In these data, "tidy" is a row for each of the seven *units* for every *student_id*.
...
reads <- import("C:/Users/Joe/Desktop/BRT/Teaching/Intro-Data-Science/2019_fall/labs/data/Project_Reads_Scores.csv", setclass = "tbl_df") %>% 
  clean_names()
